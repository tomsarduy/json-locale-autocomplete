        ...
        return vscode
          ...
          .then(files => {
            if (files.length > 0) {
              ...
              let item = new vscode.CompletionItem('FM', vscode.CompletionItemKind.Snippet);
              const autocompleteKeys = [' '].concat(Object.keys(jsonFile)).join(',');
              item.insertText = new vscode.SnippetString("<FormattedMessage id='${1|"+ autocompleteKeys +"|}' />");
